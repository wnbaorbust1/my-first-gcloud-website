...
import { toggleFavorite } from "@/lib/affirmations/affirmations";
import { affirmationActionSchema } from "@/lib/validations/affirmations";
import { assertBusinessAccess, getCurrentUser } from "@/lib/session";

/** Toggle favorite — no points (not in spec §9's table), just a saved-preference toggle. */
export async function POST(request: Request) {
  ...
  const favorited = await toggleFavorite(input.businessId, input.affirmationId);
