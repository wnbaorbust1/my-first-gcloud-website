...
export const metadata: Metadata = { title: "Scorecard — Blueprint Admin" };
...
export default async function AdminScorecardPage({
  ...
  const data = await getScorecardData(businessId);
        ...
        {!data.hasAssessment ? (
          <WorksheetPanel number={1} title="My Scores" icon="📊">
            <p className="text-sm sm:text-base">
              This business hasn&apos;t completed a Blueprint Assessment yet.
            </p>
          </WorksheetPanel>
        ) : (
          <>
            <WorksheetGrid>
              <WorksheetPanel number={1} title="My Scores" icon="📊" span="full">
                ...
                {data.healthScorePercent !== null && (
                  <div className="mt-3 flex justify-center">
                    <WorksheetStat label="Business Health" value={`${data.healthScorePercent}%`} />
                  </div>
                )}
              </WorksheetPanel>

              <WorksheetPanel number={2} title="Strength" icon="💪">
                <WorksheetChecklist>
              ...
              >
                <p className="text-lg font-bold text-legacy-700">
                  {data.recommendedSession ?? "No completed assessment"}
                </p>
              </WorksheetPanel>
            </WorksheetGrid>
